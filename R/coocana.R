...
#' @param type Charaters string, choices of species pairing types. (type = "byrow") by default. Pairing all possible pairs of species if (type = "all").
#' @param with.splits Charaters string, choices of species pairing types. (type = "byrow") by default. Pairing all possible pairs of species if (type = "all").
#' @param show.tables Logical. Display fancy tables if "TRUE".
#' @param pair.scatter Charaters string, choices of displaying scatter plots types. By default, display only the scatter plots. (pair.scatter = "fit") if the scatter plot with fitted curve is preferred. (pair.scatter = "none") if no scatter plot is needed.
#' @param any.output 	Charaters string, choices of output model types. By default, output the "lite" results. (any.output = "full") if the full result is preferred. (any.output = "none") if no result is needed.
#' @param show.split.plot Charaters string,choices of the showing plots. (show.plots = "splits") shows only the break points in the scatter plots. (show.plots = "dendrgram") shows only the tree dendrograms from the "rpart" models. (show.plots = "none") if no plot is needed. The default is (show.plots = "both"), which shows both the splits plots and the dendrograms.
#' @return Returns a list of objects inculding models, splits and tables and etc.
#' \itemize{
#'   \item Model - A list of "cooccur" models in different splited ranges for different species pairs. Or one "cooccur" model if (with.splits = FALSE).
#'   \item Splits - A list of break points in different ingradients and for different species. Or "NA" if (with.splits = FALSE).
#'   \item Stats - Statistic table containing all analyzed species pairs and probabilities for rejecting classifying positive and negative associations.
#'   \item Pairs - The used pairs of species.
#'   \item Cooc.Mat - A matrix in the form for the usage of "cooccur" function.
#'   \item Used.Ingradients	 - The used ingradients.
#' }
#' @description
#' This is the main function of the package "cooccurExtra". The main idea of this function is to do a further analysis into the cooccurence between species base on function "cooccur".
#' It helps reproduce the analysis of cooccurences between different species pairs in different ingradients. It can be used in diagnostic pairs, creating splits plots, creating matrix for the use of "cooccur" function and displaying fancy output tables.
#' @author
#' Yingjia Jot He
#' @examples
